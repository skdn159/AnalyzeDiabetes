...
xy = np.loadtxt("test.csv", delimiter=',',dtype=np.float)
...
w1 = tf.get_variable("w1", shape=[24, 36],initializer=tf.contrib.layers.xavier_initializer())
b1= tf.Variable(tf.random_normal([36]), name = 'bias1')
L1_ = tf.nn.relu(tf.matmul(x,w1)+b1)
L1 = tf.nn.dropout(L1_, keep_prob=keepProb)

#w2 = tf.Variable(tf.random_normal([36,36]),name='weight2')
w2 = tf.get_variable("w2", shape=[36, 36],initializer=tf.contrib.layers.xavier_initializer())
b2= tf.Variable(tf.random_normal([36]), name = 'bias2')
L2_ = tf.nn.relu(tf.matmul(L1,w2)+b2)
L2 = tf.nn.dropout(L2_, keep_prob=keepProb)

#w3 = tf.Variable(tf.random_normal([36,36]),name='weight3')
w3 = tf.get_variable("w3", shape=[36, 36],initializer=tf.contrib.layers.xavier_initializer())
b3= tf.Variable(tf.random_normal([36]), name = 'bias3')
L3_ = tf.matmul(L2,w3)+b3
L3 = tf.nn.dropout(L3_, keep_prob=keepProb)

#w4 = tf.Variable(tf.random_normal([36,nb_classes]),name='weight4')
w4 = tf.get_variable("w4", shape=[36, nb_classes],initializer=tf.contrib.layers.xavier_initializer())
b4= tf.Variable(tf.random_normal([nb_classes]), name = 'bias4')
L4_ = tf.matmul(L3,w4)+b4
L4 = tf.nn.dropout(L4_, keep_prob=keepProb)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(2000):
            c, _ = sess.run([cost, optimizer], feed_dict={x:x_data, y:y_data, keepProb:0.7})
    ...
    probb = sess.run(hypo, feed_dict = {x:cer_x, keepProb:1.0} )
    pred = sess.run(prediction, feed_dict = {x:cer_x,keepProb:1.0})
    ...
    print("TotalAcc = ", (float)(correctCount/idx))
